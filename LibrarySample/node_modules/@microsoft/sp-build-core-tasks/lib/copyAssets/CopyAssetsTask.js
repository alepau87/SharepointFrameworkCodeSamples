        ...
        externalModules.forEach(function (externalModule) {
            ...
            if (externalModule.chunks.reduce(function (state, chunkId) { return state || webpackChunkId === chunkId; }, false)) {
                ...
                if (localizedResource) {
                    ...
                    var defaultLocale_1;
                    var defaultLocaleInPaths_1 = false;
                    var localeKeys = Object.keys(localizedResource);
                    if (localeKeys.length === 1) {
                        // We only have one locale, so pick that one as the default
                        defaultLocale_1 = localeKeys[0];
                    }
                    else if ('default' in localizedResource) {
                        // We have a "default" default locale, so make "default" the default
                        defaultLocale_1 = 'default';
                    }
                    else if (constants_1.default.defaultLocale in localizedResource) {
                        // We have the constants.defaultLocale locale ("en-us"), so pick that one as the default
                        defaultLocale_1 = constants_1.default.defaultLocale;
                        defaultLocaleInPaths_1 = true; // In this case, the default locale is en-us, but that's also a regular path
                    }
                    else {
                        // We couldn't pick a default locale, so issue a warning
                        _this.logWarning(("Unable to select a default locale for external resource \"" + externalName + "\". ") +
                    ...
                    for (var locale in localizedResource) {
                        if (localizedResource.hasOwnProperty(locale)) {
                            ...
                            var isDefaultLocale = locale === defaultLocale_1;
                            if (isDefaultLocale) {
                                debugModule.defaultPath = path_1;
                            }
                            if (!isDefaultLocale || defaultLocaleInPaths_1) {
