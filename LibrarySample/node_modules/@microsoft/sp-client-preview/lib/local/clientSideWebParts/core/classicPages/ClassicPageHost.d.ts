/**
 * @Copyright (c) Microsoft Corporation.  All rights reserved.
 *
 * @file Web part host for classic SharePoint pages.
 */
import { ServiceScope } from '@microsoft/sp-client-base';
import IWebPartData from '../IWebPartData';
import IWebPartHost from '../../../../local/common/webParts/IWebPartHost';
import { WebPartConfigurationEvent } from '../../propertyPane/propertyPane/PropertyPaneDefinitions';
/**
 * This class represents the host for client side web parts on classic SharePoint pages.
 * @see IWebPartHost for more details on the role of a web part host.
 */
export declare class ClassicPageHost implements IWebPartHost {
    private static _instance;
    private _serviceScope;
    static getInstance(): ClassicPageHost;
    /**
     * constructor
     */
    constructor();
    /**
     * @see IWebPartHost
     */
    serviceScope: ServiceScope;
    setDirty(instanceId: string, data?: any): void;
    /**
     * @see IWebPartHost.ts
     */
    webPartConfigurationEventCallback(event: WebPartConfigurationEvent, data?: IWebPartData): void;
    private _setData(data?);
    private _ensureInitialized();
}
export default ClassicPageHost;
